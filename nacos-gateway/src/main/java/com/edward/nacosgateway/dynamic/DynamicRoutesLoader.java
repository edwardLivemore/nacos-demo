...
public class DynamicRoutesLoader implements InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        // 首次加载配置
        String routes = nacosConfigManager.getConfigService()
                .getConfig(ROUTES_CONFIG, nacosConfigProperties.getGroup(), 10000);
        dynamicRoutesListener.receiveConfigInfo(routes);

        // 注册监听器
        nacosConfigManager.getConfigService()
                .addListener(ROUTES_CONFIG, nacosConfigProperties.getGroup(), dynamicRoutesListener);
    }
}
